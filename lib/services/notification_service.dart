...
class NotificationService {
  ...
  Future<void> syncMedications(
    ...
    for (final entry in medications.asMap().entries) {
      ...
      if (!medication.isEnabled || medication.isHistoric) {
        continue;
      }
      try {
  ...
  Future<void> _ensureTimezone() async {
    ...
    try {
      ...
      final timezoneName = _resolveTimezoneName(timezone);
  ...
  Future<void> _scheduleMedication(int id, Medication medication) async {
    ...
    debugPrint(
        'NotificationService: scheduled ${medication.name} (${medication.dose}) for ${scheduleDate.toLocal()} with id $id');
  }
